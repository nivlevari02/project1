...
def plot_distance_and_expanded_wrt_weight_figure(
        ...
        weights: object,
        total_cost: object,
        total_nr_expanded: object) -> object:
    """
    Use `matplotlib` to generate a figure of the distance & #expanded-nodes
     w.r.t. the weight.
    TODO [Ex.15]: Complete the implementation of this method.
    """
    weights1, total_cost, total_nr_expanded = np.array(weights), np.array(total_cost), np.array(total_nr_expanded)
...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    for i in array:
        ...
        if res.is_solution_found:
            ...
            num_of_extanded.append(res.nr_expanded_states)
            weights.append(i)
    plot_distance_and_expanded_wrt_weight_figure(problem.name, weights, costs, num_of_extanded)
...
def toy_map_problem_experiments():
    ...
    #       solve the same `toy_map_problem` with it and print the results (as before).
    as_air = AStar(AirDistHeuristic)
    res = as_air.solve_problem(toy_map_problem)
    print(res)
    ...
    run_astar_for_weights_in_range(AirDistHeuristic, toy_map_problem) #need to be UNCOMMENTED
...
def mda_problem_with_astar_experiments():
    ...
    as_air = AStar(MDAMaxAirDistHeuristic)
    res = as_air.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
    ...
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    as_air = AStar(MDASumAirDistHeuristic)
    res = as_air.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
    ...
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    as_air = AStar(MDAMSTAirDistHeuristic)
    res = as_air.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
...
def multiple_objectives_mda_problem_experiments():
    ...
    as_test_travel = AStar(MDATestsTravelDistToNearestLabHeuristic)
    res = as_test_travel.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res)
    ...
    cost = dist_res.solution_cost
    optimal_distance_cost = 0
    if cost is not None:
        optimal_distance_cost = cost.get_g_cost()
    eps = 0.6
